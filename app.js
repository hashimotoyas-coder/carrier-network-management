        ...
        <td colspan="6">
          <div class="empty-state">
            ...
            <div class="empty-state-text">
              ${searchTerm ? '検索結果がありません' : 'マスターデータがありません'}
            </div>
          </div>
        </td>
      </tr>
      ...
      <tr>
        <td colspan="9">
          <div class="empty-state">
            <div class="empty-state-icon">📈</div>
            <div class="empty-state-text">
              ${searchTerm ? '検索結果がありません' : '進捗案件がありません'}
            </div>
          </div>
        </td>
      </tr>
...
function getStatusBadge(status) {
  const badges = {
    ...
    'progress': '<span class="badge badge-progress">進行中</span>',
    'complete': '<span class="badge badge-complete">完了</span>'
  };
...
function getPriorityBadge(priority) {
  const badges = {
    ...
    '中': '<span class="badge badge-progress">中</span>',
    '低': '<span class="badge badge-pending">低</span>'
  };
...
function editMaster(tadig) {
  ...
  document.getElementById('masterModalTitle').textContent = 'マスター編集';
...
function deleteMasterConfirm(tadig) {
  ...
  if (inUse) {
    alert('このキャリアは進捗管理表で使用されているため削除できません');
    return;
  }
  
  if (confirm(`${carrier.name} (${tadig}) を削除しますか?`)) {
...
function editProgress(sequence) {
  ...
  document.getElementById('progressModalTitle').textContent = '進捗案件編集';
...
function deleteProgressConfirm(sequence) {
  ...
  if (confirm(`案件 ${sequence} (${name}) を削除しますか?`)) {
